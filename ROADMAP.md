...
- [x] **HDR → SDR tone-mapping** — libplacebo (Vulkan, GPU) tone-maps HDR10/PQ/BT.2020 → BT.709 SDR in the transcode pass; verified output is bt709, no more washed-out grey ⭐
- [ ] **Multi-version picker** — same title in 4K HDR + 1080p 🟡
- [ ] **Offline downloads** on mobile 🟡
- [x] **GPU transcode (NVENC/NVDEC)** — config-driven (`auto`/`nvenc`/`cpu`, `NASCINEMA_TRANSCODE_HWACCEL`) with auto-probe + CPU fallback. GPU decode→HDR tonemap→NVENC encode, all on the card; live pipeline ~5× realtime, CPU off the hot path. SDR uses scale_cuda, HDR uses libplacebo ⭐
